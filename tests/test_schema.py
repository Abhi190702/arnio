...
def test_choice_invalid_values():
    import pandas as pd
...
def test_choice_empty_allowed_raises():
    with pytest.raises(ValueError, match="allowed must contain at least one value"):
        ar.Choice([])


def test_choice_bare_string_raises():
    with pytest.raises(TypeError, match="allowed must be a sequence"):
        ar.Choice("active")


def test_choice_accepts_set_and_tuple():
    assert ar.Choice({"active", "inactive"}) is not None
    assert ar.Choice(("active", "inactive")) is not None