    ...
    df = pd.DataFrame(
        {
            ...
            "revenue": np.where(
                rng.random(rows) > 0.08,
                rng.uniform(100, 99999, rows).round(2),
                None,
            ),
...
def generate_wide(
    rows=5_000,
    columns=256,
    path=DEFAULT_WIDE_PATH,
):
    if rows < 1:
        raise ValueError(
            "wide benchmark requires at least 1 row"
        )

    if columns < 4:
        raise ValueError(
            "wide benchmark requires at least 4 columns"
        )
    ...
    for index in range(columns - 1):
        ...
        if kind == 0:
            data[f"metric_{column_id}"] = (
                rng.normal(1_000, 250, rows).round(4)
            )
            ...
            data[f"amount_{column_id}"] = (
                rng.uniform(0, 10_000, rows).round(2)
            )
    ...
    print(
        f"Generated {rows:,} row x "
        f"{columns:,} column CSV -> {path}"
    )
...
def generate_multiline(
    ...
    """Generate deterministic multiline CSV benchmark dataset."""
    ...
    print(
        "Generated multiline benchmark CSV "
        f"-> {path}"
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "value": rng.uniform(0, 1000, rows).round(2),
        }
    )
